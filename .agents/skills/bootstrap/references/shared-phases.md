...
- Use main agent to implement step by step per plan in the configured `Plans:` path
- Use `ui_ux_designer` subagent for frontend per the configured `Docs:` path `design-guidelines.md`
...
Handled by **ck:cook** skill. Bootstrap-specific notes:
- `code_reviewer` subagent reviews code
- If critical issues: fix → retest → repeat
- Report summary to user when all tests pass and code reviewed

## Documentation

After code review passes. Use `docs_manager` subagent to create/update:
- the configured `Docs:` path `README.md` (≤300 lines)
- the configured `Docs:` path `codebase-summary.md`
- the configured `Docs:` path `project-overview-pdr.md` (Product Development Requirements)
- the configured `Docs:` path `code-standards.md`
- the configured `Docs:` path `system-architecture.md`

Use `project_manager` subagent to create:
- the configured `Docs:` path `project-roadmap.md`
...
3. Ask user if they want to commit/push:
   - If yes: `git_manager` subagent to commit (and push if requested)
