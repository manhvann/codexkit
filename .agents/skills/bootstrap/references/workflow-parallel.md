...
Write to the configured `Docs:` path directory (≤150 lines).

No user gate — proceed automatically.

## Step 3: Wireframe & Design

1. Use `ui_ux_designer` + `researcher` agents in parallel:
...
2. `ui_ux_designer` creates:
   - Design guidelines at the configured `Docs:` path `design-guidelines.md`
   - Wireframes in HTML at the configured `Docs:` path `wireframe/`
3. If no logo: generate with `ck:ai-multimodal` skill
4. Screenshot with `ck:chrome-devtools` → save to the configured `Docs:` path `wireframes/`
...
- Launch multiple `fullstack_developer` agents in PARALLEL for concurrent phases
  - Pass: phase file path, environment info
- Use `ui_ux_designer` for frontend (generate/analyze assets with `ck:ai-multimodal`, edit with `imagemagick`)
