...
Use `ask the user` to probe user's request, constraints, true objectives.
...
3. Present 2-3 options with pros/cons via `ask the user`
4. Write approved tech stack to the configured `Docs:` path directory
...
2. Use `ui_ux_designer` + `researcher` subagents in parallel:
...
5. Screenshot wireframes with `ck:chrome-devtools` → save to the configured `Docs:` path `wireframes/`
