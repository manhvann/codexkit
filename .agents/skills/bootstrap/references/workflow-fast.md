...
1. `ui_ux_designer` subagent analyzes research, creates:
...
3. Screenshot wireframes with `ck:chrome-devtools` → save to the configured `Docs:` path `wireframes/`
...
**Note:** Fast mode uses `git_manager` to auto-commit (no push) at the end.
