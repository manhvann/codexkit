...
2. Write tech stack to the configured `Docs:` path directory

No user gate — auto-select best option.

## Step 3: Wireframe & Design

1. Use `ui_ux_designer` + `researcher` subagents in parallel:
...
4. Screenshot wireframes with `ck:chrome-devtools` → save to the configured `Docs:` path `wireframes/`
