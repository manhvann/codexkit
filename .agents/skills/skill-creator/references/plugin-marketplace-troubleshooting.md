...
- `.codex-plugin/marketplace.json` exists at specified path?
- JSON syntax valid? Run `codex plugin validate .` or `/plugin validate .`
- Private repo — do you have access permissions?

## Validation Errors

Run `codex plugin validate .` from marketplace directory. Common errors:

| Error | Cause | Fix |
|-------|-------|-----|
| `File not found: .codex-plugin/marketplace.json` | Missing manifest | Create with required fields |
...
- Use `${CODEX_PLUGIN_ROOT}` in hooks/MCP configs for cache-aware paths
- See [Plugin caching docs](https://code.codex.com/docs/en/plugins-reference.md#plugin-caching-and-file-resolution)
