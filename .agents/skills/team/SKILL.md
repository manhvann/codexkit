...
**Requires:** Agent Teams enabled. Set `CODEX_EXPERIMENTAL_AGENT_TEAMS=1` in settings.json env if using Codex < 2.1.33. May be GA in 2.1.33+.
...
3. If the call returns an ERROR or is unrecognized: **STOP. Tell user:** "Agent Teams requires `CODEX_EXPERIMENTAL_AGENT_TEAMS=1` in settings.json. Team mode is not available."
...
4. **CALL** `Task` x N to spawn developer teammates:
   - `subagent_type: "fullstack_developer"`, `mode: "plan"`
...
4. **CALL** `Task` x N to spawn reviewers:
   - `subagent_type: "code_reviewer"`, `model: "haiku"`, `name: "reviewer-{N}"`
   - Prompt: task description + CK Context Block
