...
description: "Track progress, update plan statuses, manage runtime tasks, generate reports, coordinate docs updates. Use for project oversight, status checks, plan completion, task hydration, cross-session continuity."
argument-hint: "[task: status, hydrate, sync, report]"
---

# Project Management

Project oversight and coordination using runtime task tools with persistent plan files.
...
Use task tools when the runtime exposes them; otherwise keep checklist state in plan files.
...
- **Hydrate:** Read plan `[ ]` items → create one tracked task per unchecked item
- **Work:** task/checklist updates track progress in real time
...
- Scan the configured plans directory for active `plan.md` files
...
Trigger configured docs path updates when:
...
Delegate to `docs_manager` subagent for actual updates.
...
1. Check existing tracked tasks first when task tools exist
2. If empty: hydrate from plan files (unchecked items)
3. During work: update tracked task/checklist state as tasks progress
...
2. Reconcile every completed tracked task/checklist item to phase metadata (`phase` / `phaseFile`).
...
- `ck:cook` — Implements plans (execution phase, invokes project_manager at finalize)
