...
1. **Read plans directory:** Glob the configured `Plans:` path `*/plan.md` to discover all plans
...
tracked tasks (session-scoped): `pending` → `in_progress` → `completed`
...
2. **Code quality validated?** — code_reviewer agent report available?
