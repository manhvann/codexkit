...
Codex provides 4 native tools for session-scoped task management.
...
- `task list()` → All tasks with status, owner, blockedBy
...
3. When Agent A completes a task → `task update(status: "completed")`
