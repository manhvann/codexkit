 ...
 * Run: node .agents/skills/worktree/scripts/worktree.test.cjs
 */
...
const MONOREPO_DIR = '/home/user/codexkit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'codexkit', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from codexkit submodule
  const submodulePath = '/home/user/codexkit/codexkit';
...
test('create with partial project match in monorepo', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'codexkit-cli', 'Should find codexkit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'codexkit' matches multiple projects
  const result = run('create codexkit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
