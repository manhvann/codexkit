#!/usr/bin/env node
/**
 * Git Worktree Manager for CodexKit
 ...
 *   --worktree-root <path> Explicit worktree directory (Codex's decision)
...
// --worktree-root: explicit override for worktree location (Codex's decision)
...
// 1. Explicit --worktree-root flag (Codex's decision)
...
function getWorktreeRoot(gitRoot, isMonorepo, explicitRoot = null) {
  // Priority 0: Explicit --worktree-root flag (Codex's decision)
  if (explicitRoot) {
...
function cmdCreate() {
  ...
  if (isMonorepo) {
    ...
    if (matches.length > 1) {
      outputError('MULTIPLE_PROJECTS_MATCH', `Multiple projects match "${project}"`, {
        suggestion: 'Ask the user to select one',
  ...
  // explicitWorktreeRoot comes from --worktree-root flag (Codex's decision)
