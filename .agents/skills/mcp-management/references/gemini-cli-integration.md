...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.codex/.mcp.json`:
...
ln -sf .codex/.mcp.json .gemini/settings.json

# Create symlink (Windows - requires admin or developer mode)
mklink .gemini\settings.json .codex\.mcp.json
```
...
```bash
echo "Search for Codex AI documentation, take a screenshot of the homepage, and save both to memory" | gemini -y -m <gemini.model>
```
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.codex/.mcp.json`)
...
Should show symlink pointing to `.codex/.mcp.json`.
