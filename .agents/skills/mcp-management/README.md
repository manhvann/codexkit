...
- **Context Efficiency**: Delegate MCP operations to `mcp_manager` subagent
...
See `.codex/.mcp.json.example` for more examples.
...
Main Agent: [Spawns mcp_manager subagent]
mcp_manager: Discovers brave-search + memory tools, reports back
...
- Load config from `.codex/.mcp.json`
...
## Integration with mcp_manager Agent

The `mcp_manager` agent (`.codex/agents/mcp_manager.toml`) uses this skill to:
...
Ensure `.codex/.mcp.json` exists and is valid JSON.
