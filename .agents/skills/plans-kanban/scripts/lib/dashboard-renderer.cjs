...
function generateCardMeta(plan) {
  ...
  // Issue tag - clickable link to GitHub (uses branch to derive repo, falls back to codexkit)
  if (plan.issue) {
    // TODO: Make repo configurable via project settings
    const issueUrl = `${plan.repositoryUrl || "https://github.com/OWNER/codexkit"}/issues/${plan.issue}`;
