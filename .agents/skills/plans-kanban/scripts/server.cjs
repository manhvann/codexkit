 ...
 *   --foreground    Run in foreground (for background tasks)
 */
...
async function main() {
  ...
  // Skip if --foreground is set (for Codex background tasks)
  ...
  server.listen(port, args.host, () => {
    ...
    // In foreground mode (background task), always output JSON
    if (args.foreground || args.isChild || process.env.CODEX_COMMAND) {
      const result = {
        success: true,
        url,
        dir: plansDir,
