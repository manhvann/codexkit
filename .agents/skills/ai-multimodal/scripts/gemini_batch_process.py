...
CODEX_ROOT = Path(__file__).parent.parent.parent.parent
...
def find_api_key() -> Optional[str]:
    ...
    Uses ~/.codex/scripts/resolve_env.py for consistent resolution across all skills.
    Falls back to local resolution if centralized resolver not available.

    Priority order (highest to lowest):
    1. process.env (runtime environment variables)
    2. PROJECT/.agents/skills/ai-multimodal/.env (skill-specific)
    3. PROJECT/.agents/skills/.env (shared skills)
    4. PROJECT/.codex/.env (project global)
    5. ~/.codex/skills/ai-multimodal/.env (user skill-specific)
    6. ~/.codex/skills/.env (user shared)
    7. ~/.codex/.env (user global)
    """
    ...
    try:
        ...
        for i, generated_image in enumerate(response.generated_images):
            ...
            for parent in [script_dir] + list(script_dir.parents):
                if (parent / '.git').exists() or (parent / '.codex').exists() or (parent / '.agents').exists():
                    project_root = parent
                    break
                ...
                for i, part in enumerate(response.candidates[0].content.parts):
                    if part.inline_data:
                        ...
                        if file_path:
                            ...
                            # Find project root (look for .git or .codex directory)
                            script_dir = Path(__file__).parent
                            project_root = script_dir
                            for parent in [script_dir] + list(script_dir.parents):
                                if (parent / '.git').exists() or (parent / '.codex').exists() or (parent / '.agents').exists():
...
def batch_process(
    ...
    if not api_key:
        ...
        print("2. Show hierarchy: python ~/.codex/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
        print("3. Quick setup: export GEMINI_API_KEY='your-key'")
        print("4. Create .env: cd ~/.codex/skills/ai-multimodal && cp .env.example .env")
