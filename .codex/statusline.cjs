...
/**
 * Custom Codex Code statusline for Node.js - Multi-line Edition
...
async function main() {
  try {
    ...
    const modelName = data.model?.display_name || 'Codex';
    ...
    const billingMode = env.CODEX_BILLING_MODE || 'api';
    ...
    // Allow callers and integration tests to select a mode without mutating
    // the project configuration. Project config remains the default.
    const statuslineMode = env.CODEX_STATUSLINE_MODE || config.statusline || 'full';
