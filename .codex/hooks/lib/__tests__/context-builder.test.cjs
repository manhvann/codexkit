...
describe('context-builder.cjs', () => {
  ...
  describe('resolveRulesPath()', () => {
    ...
    it('returns null when file does not exist anywhere', () => {
      tempDir = createTempDir(['.codex']);
      process.chdir(tempDir);

      // Use a unique filename that won't exist in global ~/.codex/
  ...
  describe('Global path resolution', () => {
    ...
    it('checks global ~/.codex/rules/ path', () => {
      ...
      const tempDir = createTempDir(['.codex']);
      const originalCwd = process.cwd();
      process.chdir(tempDir);
      ...
      try {
        ...
        // Result depends on whether global ~/.codex/rules exists
  ...
  describe('AGENTS.md reference resolution', () => {
    let tempDir;
    let originalCwd;
