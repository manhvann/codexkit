 ...
 * Run: node .codex/hooks/lib/__tests__/statusline.test.cjs
 */
      ...
      content: [
        {
          ...
          id: 'agent-1',
          name: 'Task',
          input: { subagent_type: 'researcher', model: 'codex-opus', description: 'Research topic' }
        }
      ]
    }
  },
...
test('parseTranscript tracks agents correctly', async () => {
  ...
  assertEquals(agent.model, 'codex-opus', 'Should capture agent model');
});
...
test('countConfigs: returns object with expected properties', () => {
  ...
  assertTrue('codexMdCount' in result, 'Should have codexMdCount');
...
test('countConfigs: all counts are numbers', () => {
  const result = countConfigs('/tmp');
  assertEquals(typeof result.codexMdCount, 'number', 'codexMdCount should be number');
