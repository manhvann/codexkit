...
test('getGitRoot with subdirectory cwd returns same root', () => {
  ...
  // Run from .codex/hooks subdirectory
  const subdirPath = path.join(gitRoot, '.codex', 'hooks');
...
test('resolvePlanPath ignores project active plan unless project method is configured', () => {
  const tempDir = path.join(os.tmpdir(), 'ck-test-project-plan-' + Date.now());
  const stateDir = path.join(tempDir, '.codex', 'state');
  const planPath = path.join(tempDir, 'plans', '260111-stale-plan');
  fs.mkdirSync(stateDir, { recursive: true });
  fs.mkdirSync(path.dirname(planPath), { recursive: true });
  fs.mkdirSync(planPath, { recursive: true });

  try {
    fs.writeFileSync(path.join(stateDir, 'active-plan.json'), JSON.stringify({
      activePlan: planPath,
      sessionOrigin: tempDir,
      timestamp: Date.now(),
      source: 'project-state'
    }, null, 2));

    const defaultConfig = { paths: { plans: 'plans' }, plan: { resolution: { order: ['session', 'branch'] } } };
    const defaultResult = resolvePlanPath(null, defaultConfig, { cwd: tempDir });
    assertEquals(defaultResult.resolvedBy, null);
    assertEquals(defaultResult.path, null);

    const projectConfig = { paths: { plans: 'plans' }, plan: { resolution: { order: ['project'] } } };
    const projectResult = resolvePlanPath(null, projectConfig, { cwd: tempDir });
    assertEquals(projectResult.resolvedBy, 'project');
    assertEquals(projectResult.path, planPath);
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
});
