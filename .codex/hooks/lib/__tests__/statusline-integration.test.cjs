...
try {
  const result = execSync(`echo '${minimalInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
  ...
  test('Minimal output contains model name', () => {
    assertContains(result, 'Codex', 'Should contain model name');
  });
...
try {
  ...
  const gitInput = JSON.stringify({
    model: { display_name: 'Codex-3' },
    workspace: { current_dir: tmpDir },
    context_window: { context_window_size: 200000 }
  });

  const gitResult = execSync(`echo '${gitInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    cwd: tmpDir,
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const contextResult = execSync(`echo '${contextInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
process.env.CODEX_BILLING_MODE = 'api';
...
try {
    const costResult = execSync(`echo '${costInput.replace(/'/g, "'\\''")}'  | CODEX_BILLING_MODE=api node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
    env: { ...process.env, CODEX_BILLING_MODE: 'api' }
  });
...
try {
  const invalidResult = execSync(`echo 'not valid json'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const emptyResult = execSync(`echo '' | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const multilineResult = execSync(`echo '${multilineInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
  ...
  test('Output contains session line', () => {
    assertContains(multilineResult, 'Codex-Opus', 'Should contain model info');
  });
...
try {
  const expandResult = execSync(`echo '${expandInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  ...
  const noColorResult = execSync(`echo '${colorInput.replace(/'/g, "'\\''")}'  | NO_COLOR=1 node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
let wideLines = 0;
try {
  const wideResult = execSync(`echo '${wideInput.replace(/'/g, "'\\''")}'  | COLUMNS=160 node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
try {
  const narrowResult = execSync(`echo '${narrowInput.replace(/'/g, "'\\''")}'  | COLUMNS=80 node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
try {
  const longPathResult = execSync(`echo '${longPathInput.replace(/'/g, "'\\''")}'  | COLUMNS=100 node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
...
const longModelInput = JSON.stringify({
  model: { display_name: 'gemini-codex-opus-4-5-thinking-extended-context' },
  workspace: { current_dir: '/home/user/project' },
  context_window: { context_window_size: 200000, current_usage: { input_tokens: 50000 } }
});

try {
  const longModelResult = execSync(`echo '${longModelInput.replace(/'/g, "'\\''")}'  | COLUMNS=100 node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
    env: { ...process.env, COLUMNS: '100' }
  });
...
try {
  const agentTodoResult = execSync(`echo '${agentTodoInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const zeroResult = execSync(`echo '${zeroContextInput.replace(/'/g, "'\\''")}'  | node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe']
  });
...
try {
  const tinyResult = execSync(`echo '${wideInput.replace(/'/g, "'\\''")}'  | COLUMNS=40 node .codex/statusline.cjs`, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
