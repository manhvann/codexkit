...
grep -A5 "console.log.*TEST.*:" .codex/hooks/lib/__tests__/statusline.test.cjs
```
...
{"timestamp":"2026-01-06T12:03:00Z","message":{"content":[{"type":"tool_use","id":"agent-1","name":"Task","input":{"subagent_type":"researcher","model":"codex-opus","description":"Research topic"}}]}}
...
```
[Codex-Opus] | ██████░░░░░░ 48% | 📁 ~/project | 📝 +42 -12 | 💵 $0.1234
```
...
```bash
# Test only colors
node -e "const c = require('./.codex/hooks/lib/colors.cjs'); console.log(c.coloredBar(50))"

# Test only parser
node -e "const {parseTranscript} = require('./.codex/hooks/lib/transcript-parser.cjs'); console.log('Parser loaded')"

# Test only config counter
node -e "const {countConfigs} = require('./.codex/hooks/lib/config-counter.cjs'); console.log(countConfigs('/tmp'))"
```
